...
"""Graph orchestration behind :func:`observe_node`.

This module drives the live ROS graph: it waits for the target node to appear,
polls until its endpoint set settles, and feeds the collected raw data to the
pure builders in :mod:`._endpoints` / :mod:`._parameters` / :mod:`._qos`.  The
package re-exports :func:`observe_node` and :class:`NodeNotFoundError` from
here; nothing else in here is public API.
"""
