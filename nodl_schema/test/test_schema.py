...
def test_service_with_qos():
    validate({
        "nodl_version": 2,
        "service_servers": [
            {"name": "/set_bool", "type": "std_srvs/srv/SetBool", "qos": _MIN_QOS}
        ],
    })
...
def test_publisher_missing_name():
    with pytest.raises(ValidationError):
        validate({"nodl_version": 2,
                  "publishers": [{"type": "std_msgs/msg/String", "qos": _MIN_QOS}]})


def test_publisher_missing_qos():
    with pytest.raises(ValidationError):
        validate({"nodl_version": 2,
                  "publishers": [{"name": "/t", "type": "std_msgs/msg/String"}]})
...
def test_action_missing_name():
    with pytest.raises(ValidationError):
        validate({"nodl_version": 2,
                  "action_servers": [{"type": "nav2_msgs/action/NavigateToPose"}]})
