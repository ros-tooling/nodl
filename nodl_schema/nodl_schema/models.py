...
class Base(Enum):
    """
    Built-in ROS 2 base node type whose interface this node inherits.
    Resolves to a built-in interface definition shipped with ``nodl_schema``.
...
class Interface(BaseModel):
    ...
    servers/clients, and action servers/clients. An interface definition
    may be partial; whole nodes are assembled from interface definitions by
    the node definition schema (``node.schema.yaml``) via base + mixins +
    main. Field names align with ``rcl_interfaces`` and ``rosgraph_msgs``
    (``Topic``, ``Service``, ``Action``) so an interface definition maps
...
class Node(BaseModel):
    ...
    - ``base`` -- an optional built-in ROS 2 node type whose interface is inherited.
    - ``main`` -- the interface this node's implementation owns (an interface definition).
    - ``mixins`` -- zero or more additional interface definitions merged in by
      reference or in place.

    Code generation works *forward* from ``base`` + ``main`` and ignores
    ``mixins``. Documentation and conformance testing use the fully merged
    interface (base, then mixins in order, then main; later layers win).

    When working *backward* from an observed node, ``base`` can be deduced and
    the full non-base interface placed in ``main``; ``mixins`` cannot be deduced
    ...
    main: Interface = Field(
        ...,
        description="The interface this node's implementation owns, as an in-place interface\ndefinition. Code generation implements exactly this layer.\n",
    )
    mixins: Optional[list[Union[str, Interface]]] = Field(
        None,
        description="Additional interface definitions merged into this node's interface for\ndocumentation and conformance, but ignored by code generation.\nEach entry is either a reference -- an ament-index locator\n``nodl://<package>/<name>`` or a path relative to this file -- or an\nin-place interface definition (an escape hatch when no registered\ninterface definition exists).\n",
    )
