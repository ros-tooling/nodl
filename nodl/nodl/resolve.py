...
def _load_builtin(base: str) -> NodlDocument:
    ...
    path = ir.files('nodl') / 'resources' / 'fragments' / f'{base}.nodl.yaml'
...
def _load_ament_fragment(pkg: str, name: str) -> NodlDocument:
    try:
        ...
        raise ImportError(
            'ament_index_python is required to resolve nodl:// URIs'
        ) from exc
...
def _resolve_ref(ref: str, source_path: Optional[Path] = None) -> NodlDocument:
    ...
    if source_path is None:
        raise ValueError(
            f'Cannot resolve relative fragment ref {ref!r} without a source path'
        )
...
def resolve(
    ...
    if doc.base is not None:
        if doc.base not in _BUILTIN_BASES:
            raise ValueError(
                f'Unknown base {doc.base!r}. Must be one of: {sorted(_BUILTIN_BASES)}'
            )
        base_doc = _load_builtin(doc.base)
