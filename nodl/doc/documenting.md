...
The `nodl_sphinx` extension renders that document into your package's documentation while the docs are being built,
...
{doc}`nodl_sphinx package page <_generated/packages/nodl_sphinx/overview>`.

## 1. Depend on the extension

`nodl_sphinx` is only needed when documentation is built, which is what `doc_depend` means:
...
- The {doc}`nodl_sphinx page <_generated/packages/nodl_sphinx/overview>` documents the directive's argument forms,
