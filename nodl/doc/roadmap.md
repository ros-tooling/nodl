...
1. "Generate": Taking a NoDL document as input, produce a node with that interface, allowing the author to skip the boilerplate and write only the business logic. C++ (`rclcpp`) generation is available through `nodl_generator_cpp`. Initial Python (`rclpy`) interface generation is available through `nodl_generator_py`. Rust (`rclrs`) remains a stretch goal.
