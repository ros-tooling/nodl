...
1. "Generate": Taking a NoDL document as input, produce a node with that interface, allowing the author to skip the boilerplate and write only the business logic. Python (`rclpy`) generation is available through `nodl_generator_py`; C++ (`rclcpp`) and, as a stretch goal, Rust (`rclrs`) remain targets.
