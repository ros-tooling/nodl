# SPDX-FileCopyrightText: 2026 Open Source Robotics Foundation, Inc.
# SPDX-License-Identifier: Apache-2.0

from dataclasses import dataclass
from pathlib import Path
from typing import Optional

from nodl_generator_common.provenance import (
    build_provenance_map,
    filter_provided_entities,
    resolve_provenance,
)
from nodl_schema import dump_nodl
from nodl_schema.loader import DocumentTree, IncludedDocument
from nodl_schema.models import (
    ActionEndpoint,
    History,
    NodlDocument,
    QosProfile,
    Reference,
    Reliability,
    ServiceEndpoint,
    TopicEndpoint,
)

_QOS = QosProfile(history=History.SYSTEM_DEFAULT, reliability=Reliability.SYSTEM_DEFAULT)


# ---------------------------------------------------------------------------
# Fake language config + extractor
#
# The barrier walk is generic over the config type. These tests drive it with a
# trivial sentinel config keyed off a ``fake`` marker in ``codegen`` rather than
# any real generator's model, proving the core is language-agnostic.
# ---------------------------------------------------------------------------


@dataclass
class FakeConfig:
    name: str


def _fake_extractor(doc: NodlDocument) -> Optional[FakeConfig]:
    if doc.codegen is None:
        return None
    return FakeConfig(name=doc.codegen['fake']['name'])


def _barrier_codegen(name='base'):
    return {'fake': {'name': name}}


def _topic(name, type_='std_msgs/msg/String') -> TopicEndpoint:
    return TopicEndpoint(name=name, type=type_, qos=_QOS)


def _service(name, type_='std_srvs/srv/Trigger') -> ServiceEndpoint:
    return ServiceEndpoint(name=name, type=type_)


def _action(name, type_='example_interfaces/action/Fibonacci') -> ActionEndpoint:
    return ActionEndpoint(name=name, type=type_)


def _included(ref, doc, children=None):
    path = Path(f'{ref.lstrip("test://")}.yaml')
    return IncludedDocument(ref=ref, path=path, doc=doc, resolved_includes=children or [])


def _tree(root, children=None):
    return DocumentTree(root_doc=root, resolved_includes=children or [])


# ---------------------------------------------------------------------------
# No includes
# ---------------------------------------------------------------------------


def test_empty_tree():
    tree = _tree(NodlDocument())
    barriers, entity_map = build_provenance_map(tree, _fake_extractor)
    assert barriers == []
    assert entity_map == {}


def test_root_entities_not_in_map():
    root = NodlDocument(publishers=[_topic('/status')])
    tree = _tree(root)
    barriers, entity_map = build_provenance_map(tree, _fake_extractor)
    assert barriers == []
    assert entity_map == {}


# ---------------------------------------------------------------------------
# Single barrier
# ---------------------------------------------------------------------------


def test_single_barrier():
    base_doc = NodlDocument(
        codegen=_barrier_codegen('base'),
        publishers=[_topic('/rosout')],
    )
    root = NodlDocument(publishers=[_topic('/status')])
    tree = _tree(root, [_included('test://base', base_doc)])

    barriers, entity_map = build_provenance_map(tree, _fake_extractor)

    assert len(barriers) == 1
    assert barriers[0].name == 'base'
    assert entity_map == {('publishers', '/rosout'): barriers[0]}


def test_all_entity_types_collected():
    base_doc = NodlDocument(
        codegen=_barrier_codegen(),
        publishers=[_topic('/pub')],
        subscriptions=[_topic('/sub')],
        service_servers=[_service('/srv_server')],
        service_clients=[_service('/srv_client')],
        action_servers=[_action('/act_server')],
        action_clients=[_action('/act_client')],
        parameters={'my_param': {'type': 'int', 'default_value': 1}},
    )
    tree = _tree(NodlDocument(), [_included('test://base', base_doc)])

    barriers, entity_map = build_provenance_map(tree, _fake_extractor)

    assert len(barriers) == 1
    expected_keys = {
        ('publishers', '/pub'),
        ('subscriptions', '/sub'),
        ('service_servers', '/srv_server'),
        ('service_clients', '/srv_client'),
        ('action_servers', '/act_server'),
        ('action_clients', '/act_client'),
        ('parameters', 'my_param'),
    }
    assert set(entity_map.keys()) == expected_keys
    assert all(v is barriers[0] for v in entity_map.values())


# ---------------------------------------------------------------------------
# Inheritance chain (barrier absorbs descendants)
# ---------------------------------------------------------------------------


def test_chain_produces_single_barrier():
    """An outer barrier including an inner barrier absorbs the whole subtree."""
    inner_doc = NodlDocument(
        codegen=_barrier_codegen('inner'),
        publishers=[_topic('/rosout')],
    )
    outer_doc = NodlDocument(
        codegen=_barrier_codegen('outer'),
        publishers=[_topic('/transition_event')],
    )
    outer_inc = _included(
        'test://outer',
        outer_doc,
        [_included('test://inner', inner_doc)],
    )
    tree = _tree(NodlDocument(), [outer_inc])

    barriers, entity_map = build_provenance_map(tree, _fake_extractor)

    assert len(barriers) == 1
    assert barriers[0].name == 'outer'
    # Both the outer barrier's own and the inner's entities are attributed to outer.
    assert ('publishers', '/transition_event') in entity_map
    assert ('publishers', '/rosout') in entity_map
    assert all(v.name == 'outer' for v in entity_map.values())


# ---------------------------------------------------------------------------
# Non-barrier passthrough
# ---------------------------------------------------------------------------


def test_barrier_grandchild():
    """A plain include wrapping a barrier still finds the barrier."""
    base_doc = NodlDocument(
        codegen=_barrier_codegen(),
        publishers=[_topic('/rosout')],
    )
    wrapper_doc = NodlDocument(
        subscriptions=[_topic('/wrapper_sub')],
    )
    wrapper_inc = _included(
        'test://wrapper',
        wrapper_doc,
        [_included('test://base', base_doc)],
    )
    tree = _tree(NodlDocument(), [wrapper_inc])

    barriers, entity_map = build_provenance_map(tree, _fake_extractor)

    assert len(barriers) == 1
    # Barrier claims its own entities.
    assert ('publishers', '/rosout') in entity_map
    # Wrapper's entities are NOT behind a barrier (wrapper has no codegen).
    assert ('subscriptions', '/wrapper_sub') not in entity_map


# ---------------------------------------------------------------------------
# Multiple barriers
# ---------------------------------------------------------------------------


def test_two_sibling_barriers():
    base_a = NodlDocument(
        codegen=_barrier_codegen('A'),
        publishers=[_topic('/a_topic')],
    )
    base_b = NodlDocument(
        codegen=_barrier_codegen('B'),
        publishers=[_topic('/b_topic')],
    )
    tree = _tree(
        NodlDocument(),
        [
            _included('test://a', base_a),
            _included('test://b', base_b),
        ],
    )

    barriers, entity_map = build_provenance_map(tree, _fake_extractor)

    assert len(barriers) == 2
    assert ('publishers', '/a_topic') in entity_map
    assert ('publishers', '/b_topic') in entity_map


def test_duplicate_config_still_two_barriers():
    base_a = NodlDocument(
        codegen=_barrier_codegen('same'),
        publishers=[_topic('/a_topic')],
    )
    base_b = NodlDocument(
        codegen=_barrier_codegen('same'),
        publishers=[_topic('/b_topic')],
    )
    tree = _tree(
        NodlDocument(),
        [
            _included('test://a', base_a),
            _included('test://b', base_b),
        ],
    )

    barriers, entity_map = build_provenance_map(tree, _fake_extractor)

    assert len(barriers) == 2


# ---------------------------------------------------------------------------
# filter_provided_entities
# ---------------------------------------------------------------------------


def test_filter_empty_map_keeps_everything():
    merged = NodlDocument(
        publishers=[_topic('/pub')],
        subscriptions=[_topic('/sub')],
        service_servers=[_service('/srv_server')],
        service_clients=[_service('/srv_client')],
        action_servers=[_action('/act_server')],
        action_clients=[_action('/act_client')],
        parameters={'my_param': {'type': 'int', 'default_value': 1}},
    )

    filtered = filter_provided_entities(merged, {})

    assert [p.name for p in filtered.publishers] == ['/pub']
    assert [s.name for s in filtered.subscriptions] == ['/sub']
    assert [s.name for s in filtered.service_servers] == ['/srv_server']
    assert [s.name for s in filtered.service_clients] == ['/srv_client']
    assert [a.name for a in filtered.action_servers] == ['/act_server']
    assert [a.name for a in filtered.action_clients] == ['/act_client']
    assert list(filtered.parameters) == ['my_param']


def test_filter_removes_entities_behind_barrier():
    merged = NodlDocument(
        publishers=[_topic('/kept'), _topic('/provided')],
        parameters={'kept_param': {'type': 'int', 'default_value': 1}, 'provided_param': {'type': 'int'}},
    )
    provenance_map = {
        ('publishers', '/provided'): object(),
        ('parameters', 'provided_param'): object(),
    }

    filtered = filter_provided_entities(merged, provenance_map)

    assert [p.name for p in filtered.publishers] == ['/kept']
    assert list(filtered.parameters) == ['kept_param']


def test_filter_empty_document():
    filtered = filter_provided_entities(NodlDocument(), {})

    assert filtered.publishers == []
    assert filtered.subscriptions == []
    assert filtered.service_servers == []
    assert filtered.service_clients == []
    assert filtered.action_servers == []
    assert filtered.action_clients == []
    assert filtered.parameters == {}


# ---------------------------------------------------------------------------
# resolve_provenance (load -> provenance -> filter, from a real file)
# ---------------------------------------------------------------------------


def _write(path: Path, doc: NodlDocument) -> Path:
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(dump_nodl(doc))
    return path


def test_resolve_provenance_from_file(tmp_path: Path):
    base = _write(
        tmp_path / 'base.nodl.yaml',
        NodlDocument(codegen=_barrier_codegen('base'), publishers=[_topic('/rosout')]),
    )
    root = _write(
        tmp_path / 'node.nodl.yaml',
        NodlDocument(
            publishers=[_topic('/status')],
            include=[Reference(ref='local://base.nodl.yaml')],
        ),
    )

    resolved = resolve_provenance(root, _fake_extractor)

    # The included document is a barrier that owns its own entities.
    assert [b.name for b in resolved.barriers] == ['base']
    # Only the root's own entity survives filtering; the barrier's does not.
    assert [p.name for p in resolved.entities.publishers] == ['/status']
    # Sources are the resolved root plus every included path.
    assert resolved.sources == [root.resolve(), base.resolve()]
