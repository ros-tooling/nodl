...
from nodl_sphinx.directives import (
    ACTION_COLUMNS,
    ENDPOINT_COLUMNS,
    PARAMETER_COLUMNS,
    build_node_section,
...
from nodl_sphinx.summarize import ActionRow, EndpointRow, NodeSummary, ParameterRow
...
def _project(
    ...
    *,
    ...
    extensions: tuple[str, ...] = ('nodl_sphinx',),
) -> Path:
...
def test_the_directive_works_from_a_markdown_source(make_app, tmp_path: Path):
    ...
    source = _project(tmp_path, body=MYST_PAGE, suffix='.md', extensions=('myst_parser', 'nodl_sphinx'))
    app = _build_clean(make_app, source, tmp_path)
